<template>
  <q-page class="q-pa-md">
    <q-card class="report-card">
      <q-card-section>
        <div class="text-h5 text-weight-bold q-mb-md">ক্ষেত্র আবেদনের প্রতিবেদন তৈরি করুন</div>
        ...
        <div class="office-selection-container">
          ...
          <div v-if="salarySummary" class="report-generation-section q-mt-lg">
            <q-card flat bordered>
              <q-card-section>
                <div class="text-h6">বেতন সারসংক্ষেপ প্রস্তুত হয়েছে</div>
                <div class="text-caption q-mb-md">
                  {{ finalSelection.name }}-এ {{ employees.length }} জন কর্মচারীর জন্য
                </div>
                
                <q-btn
                  label="ক্ষেত্র আবেদনের প্রতিবেদন ডাউনলোড করুন"
                  color="primary"
                  icon="picture_as_pdf"
                  @click="generatePdf"
...
const fetchOfficeHierarchy = async () => {
  try {
    ...
    $q.notify({
      type: 'negative',
      message: 'অফিসের স্তরভিত্তিক কাঠামো লোড করতে ব্যর্থ হয়েছে'
    })
  }
}
...
const calculateSalarySummary = (employees, office) => {
  ...
  employees.forEach(emp => {
    ...
    const optinalFees= emp.employee?.data?.currentSalary?.data?.additional_info;
    ...
    if (salaryData) {
      ...
      const education = optinalFees.education_fee;//apatoto increment theke nicci pore optinal fees koto ta theke nite hbe
      const tiffin = optinalFees.tiffin_fee;
      const washing = optinalFees.washing_fee;
...
const generatePdf = async () => {
  ...
  try {
    ...
    $q.notify({
      type: 'positive',
     message: 'ক্ষেত্র আবেদনের প্রতিবেদন সফলভাবে তৈরি হয়েছে!'
    })
  } catch (error) {
    $q.notify({
      type: 'negative',
      message: 'প্রতিবেদন তৈরি করতে ব্যর্থ হয়েছে'
