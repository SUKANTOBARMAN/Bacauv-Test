<template>
  <q-page class="q-pa-sm">
    <q-card class="no-shadow" bordered>
      <q-card-section>
        <div class="text-h6 text-indigo-8">Reward Types</div>
        <div class="text-subtitle2">List of all Reward Types</div>
      </q-card-section>
    </q-card>
    ...
    <div class="q-pa-none">
      <div class="row q-col-gutter-md">
        ...
        <div class="col-5">
          <q-card>
            <q-card-section>
              <div class="text-h6 text-indigo-8">{{ isEditing ? 'Edit' : 'Add' }} Reward Types</div>
...
const fetchLeaves = async () => {
  try {
    const { data } = await api.get('/v1/categories?search=type:reward_type')
...
const onSubmit = async () => {
  try {
    ...
    $q.notify({ message: 'Reward saved successfully', color: 'positive', icon: 'check' })
    onReset()
    await fetchLeaves()
  } catch (error) {
...
const deleteItem = async (row) => {
  $q.dialog({
    ...
    try {
      ...
      $q.notify({ message: 'Reward deleted', color: 'negative', icon: 'check' })
      await fetchLeaves()
    } catch (error) {
