<template>
  <q-page class="q-pa-md">
    <q-card class="office-selection-card">
      <q-card-section>
        <div class="text-h5 text-weight-bold q-mb-md">Select Office</div>
        
        <!-- Multi-level Office Selection -->
        <div class="office-selection-container">
          ...
          <div v-if="salarySummary" class="salary-summary q-mt-lg" ref="printArea">
            <div class="row justify-between items-center q-mb-md">
              <div class="text-h6">Yearly Salary Summary</div>
              <q-btn 
                label="Download PDF" 
                color="primary" 
                icon="picture_as_pdf"
            ...
            <q-card flat bordered>
              <q-card-section>
                ...
                <q-list bordered separator class="summary-list">
                  <q-item>
                    <q-item-section>
                      <q-item-label>Total Basic Salary</q-item-label>
                    </q-item-section>
                    <q-item-section side>
                      <q-item-label>{{ formatCurrency(salarySummary.totalBasic) }}</q-item-label>
                    </q-item-section>
                  </q-item>
                  
                  <q-item>
                    <q-item-section>
                      <q-item-label>Total Rent Fee</q-item-label>
                      <q-item-label caption>
                        Calculated based on city and salary scale
                      </q-item-label>
                    </q-item-section>
                    <q-item-section side>
                      <q-item-label>{{ formatCurrency(salarySummary.totalRent) }}</q-item-label>
                    </q-item-section>
                  </q-item>
                  
                  <q-item>
                    <q-item-section>
                      <q-item-label>Total Medical Fee</q-item-label>
                    </q-item-section>
                    <q-item-section side>
                      <q-item-label>{{ formatCurrency(salarySummary.totalMedical) }}</q-item-label>
                    </q-item-section>
                  </q-item>
                  
                  <q-item>
                    <q-item-section>
                      <q-item-label>Total Education Fee</q-item-label>
                    </q-item-section>
                    <q-item-section side>
                      <q-item-label>{{ formatCurrency(salarySummary.totalEducation) }}</q-item-label>
                    </q-item-section>
                  </q-item>
                  
                  <q-item>
                    <q-item-section>
                      <q-item-label>Total Tiffin Fee</q-item-label>
                    </q-item-section>
                    <q-item-section side>
                      <q-item-label>{{ formatCurrency(salarySummary.totalTiffin) }}</q-item-label>
                    </q-item-section>
                  </q-item>
                  
                  <q-item>
                    <q-item-section>
                      <q-item-label>Total Washing Fee</q-item-label>
                    </q-item-section>
                    <q-item-section side>
                      <q-item-label>{{ formatCurrency(salarySummary.totalWashing) }}</q-item-label>
                    </q-item-section>
                  </q-item>
                  
                  <q-item class="total-row">
                    <q-item-section>
                      <q-item-label class="text-weight-bold">Total Yearly Salary</q-item-label>
                    </q-item-section>
                    <q-item-section side>
...
const downloadPDF = () => {
  ...
  const documentDefinition = {
    info: {
      title: `Salary Summary - ${finalSelection.value.name}`,
    ...
    content: [
      {
        text: 'Yearly Salary Summary Report',
        fontSize: 18,
        bold: true,
        alignment: 'center',
        margin: [0, 0, 0, 10]
      ...
      {
        text: `Type: ${formatOfficeType(finalSelection.value.type)}${
          finalSelection.value.additional_info?.city_corporation 
            ? ` | City: ${finalSelection.value.additional_info.city_corporation}` 
            : ''
        }`,
        fontSize: 12,
        margin: [0, 0, 0, 10]
  ...
  pdfMake.createPdf(documentDefinition).download(`salary-summary-${finalSelection.value.name}.pdf`)
}
